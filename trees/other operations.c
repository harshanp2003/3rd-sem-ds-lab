...
struct root *maximum(struct root *r)
{
   if(r==NULL)
       return r;
   struct root *cur;
...
struct root *minimum(struct root *r)
{
   if(r==NULL)
       return r;
   struct root *cur;
...
int main()
{
    struct root *m,*r=malloc(sizeof(struct root));
    m=malloc(sizeof(struct root));
    int key,f,data,ch;
    ...
    while(1)
  {
        ...
        switch(ch)
    {
       ...
       case 4:printf("POst order is\n");
        ...
        case 8:exit(0);
        case 9:m=maximum(r);
               printf("%d is the maximum data\n",m->data);
        case 10:m=minimum(r);
                printf("%d is the minimum data \n",m->data);
       default:printf("Invalid choice\n");
               break;
    }
  }
